...
class _EventDetailPageState extends State<EventDetailPage>
{
  ...
  Widget build(BuildContext context) {
    ...
    (ss.where((elem) => elem == EventName).isNotEmpty) ?  isSyb = true : isSyb = false;
    return Scaffold(
        appBar: AppBar(
        ...
        body: Padding(
          ...
          child: ListView(
            children: [
              Container(
                child: EventPhoto != "" ? Image.network(EventPhoto,
                  fit: BoxFit.cover,
                ) : Image.asset(
                    ...
                    onPressed: () async {
                      setState(() {
                        if (isSyb == false)
                          ...
                          ss.remove(EventName);
                        FirebaseFirestore.instance.collection("users").doc(uidCur).update({"subs": ss});
                      });
                    },
                    child: Text(isSyb == false ? 'Добавить в подписки': 'Убрать из подписок'),
                  ),
                ),
              ),
              const SizedBox(
